...
static func search_lead(state) -> String:
	ensure_longevity_state(state)
	var costs := {"intel": 80, "immortal_stone": 120}
...
static func npc_longevity_action(state, npc: Dictionary, world_logs: Array) -> bool:
	...
	if roll <= 48:
		var post: Dictionary = market_lead_post(state, npc, "NPC求生")
		state.market_posts.push_front(post)
		npc["last_action"] = "发布寿蛊线索订单"
		world_logs.append("NPC求生：%s发布寿蛊线索订单。" % String(npc.get("name", "某人")))
		return true
	if roll <= 76:
		_spawn_lifespan_hunt_event(state, "低寿元NPC夺寿")
